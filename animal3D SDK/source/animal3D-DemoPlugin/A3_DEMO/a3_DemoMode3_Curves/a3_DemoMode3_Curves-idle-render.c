...
a3ret a3vertexDrawableRenderIsoPatches(a3ui32 const count)
{
	if (count)
	{
		...
		float const tessLevelOuter[4] = { 1.0f, 1.0f, 1.0f, 1.0f }, tessLevelInner[2] = { 1.0f,1.0f };
		glPatchParameteri(GL_PATCH_VERTICES, 2); //tell openGL we are drawing lines | define what a patch is
		...
		glBindVertexArray(0); //turn off geometry
		glBindBuffer(GL_ARRAY_BUFFER, 0);//turn off bound buffers for safety
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);
		glDrawArrays(GL_PATCHES, 0, count * 2);//invoke drawing | 2 segments per patch = count * 2
		return 1;
	}
	return -1;
}
...
a3ret a3vertexDrawableRenderTriPatches(a3_VertexDrawable const* drawable)
{
	...
	if (drawable)
	{
		...
		// draw
		glPatchParameteri(GL_PATCH_VERTICES, 3); //tell openGL we are drawing triangles | define what a patch is
